...
use crate::types::{info_bits, CodeInitParams};
...
impl AdditiveCode for LdpcCode {
    fn setup(params: CodeInitParams) -> Self {
        ...
        let arithmetic = params
            .decoder_type
            .unwrap_or(DecoderImplementation::Aminstarf32);
...
mod tests {
    ...
    fn test_ldpc_setup_dimensions() {
        ...
        assert_eq!(
            code.input_length(),
            1024,
            "Input length should be 1024 for K1024"
        );
        assert!(
            code.output_length() > code.input_length(),
    ...
    fn test_ldpc_encode_zero_message() {
        ...
        for bit in encoded.iter() {
            assert!(
                bit.is_zero(),
                "Encoding zero message should produce zero codeword"
            );
        }
    }
    ...
    fn test_ldpc_encode_decode_roundtrip_no_erasures() {
        ...
        assert!(
            ...
            "Decoding should succeed with no erasures"
        );

        for (i, &original_bit) in message_vec.iter().enumerate() {
            let decoded_bit = if decode_result.codeword[i] == 1 {
                GF2::one()
            } else {
                GF2::zero()
            };
            assert_eq!(
                original_bit, decoded_bit,
                "Decoded bit {} should match original",
                i
            );
        }
    }
    ...
    fn test_ldpc_decode_with_small_erasures() {
        ...
        assert!(
            decode_result.success,
            "Decoding should succeed with small erasures"
        );
    }
