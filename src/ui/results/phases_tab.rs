...
use crate::benchmark::{BenchmarkSummary, BenchmarkParams, BenchmarkStats, PhaseStats};
use crate::types::DecodingStats;
use crate::ui::localization::Localization;
use crate::ui::constants::{self, heading_size, small_size};
use super::utils::{format_duration, show_phase_pie_chart, compare_benchmark_params};
...
impl PhasesTab {
        ...
        ScrollArea::vertical().show(ui, |ui| {
            if let Some(ref summary) = self.summary {
                ...
                if !summary.deal_stats.is_empty() {
                    self.show_stats_section(ui, &summary.deal_stats, "deal", "deal_phases_title", false);
                }
                
                ui.add_space(constants::SECTION_SPACING);
                
                if !summary.reconstruct_stats.is_empty() {
                    self.show_stats_section(ui, &summary.reconstruct_stats, "reconstruct", "reconstruct_phases_title", true);
                }
            }
        });
    }
    ...
    fn show_stats_section(
        ...
        for (params, stats) in entries {
            let Some(phase_metrics) = &stats.phase_metrics else { continue };
    ...
    fn show_phase_details(
        ...
        show_phase_pie_chart(ui, phase_metrics, self.localization.get("phase_distribution"));
    }
    
    fn show_decoding_stats(
        &self,
        ui: &mut Ui,
