...
fn draw_param_labels(plot_ui: &mut plot::PlotUi, param_labels: &[(f64, String)], y_max: f64, label_size: f32) {
...
fn draw_chart_title(plot_ui: &mut plot::PlotUi, title: &str, x_center: f64, y_max: f64, title_size: f32) {
    plot_ui.text(plot::Text::new(
        "",
...
impl VisualizationTab {
    ...
    fn show_bar_chart(&self, ui: &mut Ui, summary: &BenchmarkSummary, plot_height: f32) {
        ui.push_id("bar_chart_section", |ui| {
            ...
            self.build_plot(ui, "bar_chart_plot", plot_height).show(ui, |plot_ui| {
                ...
                draw_chart_title(plot_ui, &chart_comparison_title, bar_index / 2.0, y_max, title_size);
    ...
    fn show_line_chart(&self, ui: &mut Ui, summary: &BenchmarkSummary, plot_height: f32) {
        ui.push_id("line_chart_section", |ui| {
            ...
            self.build_plot(ui, "line_chart_plot", plot_height).show(ui, |plot_ui| {
                let mut entries: Vec<_> = summary.total_stats.iter().collect();
                entries.sort_by(|a, b| compare_benchmark_params(a.0, b.0));
                ...
                for (params, stats) in &entries {
                    ...
                    // Each unique (rate, info_size, decoder, c_value) configuration gets one X position,
                    // shared by both sequential and parallel implementations.
                    let config_key = format!("{:?}_{:?}_{:?}_{}",
                        params.ldpc_rate, params.ldpc_info_size, params.decoder_type, params.c_value);
                ...
                if !seq_points.is_empty() {
                    plot_ui.line(
                        plot::Line::new(&legend_sequential, plot::PlotPoints::from(seq_points.clone()))
                            .color(constants::sequential_color())
                            .width(2.5),
                    );
                    plot_ui.points(
                ...
                if !par_points.is_empty() {
                    plot_ui.line(
                        plot::Line::new(&legend_parallel, plot::PlotPoints::from(par_points.clone()))
                            .color(constants::parallel_color())
                            .width(2.5),
                    );
                    plot_ui.points(
                ...
                draw_chart_title(plot_ui, &chart_comparison_title, config_index / 2.0, y_max, title_size);
                draw_param_labels(plot_ui, &param_labels, y_max, label_size);
