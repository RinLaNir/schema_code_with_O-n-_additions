use super::table_builder::{phase_detail_columns, ResultsTable};
...
fn draw_duration_with_bar(
    ...
    if ui.is_rect_visible(rect) {
        ...
        painter.rect_filled(
            rect,
            constants::DATA_BAR_CORNER_RADIUS,
            constants::data_bar_bg(ui),
        );
        ...
        if bar_width > 0.5 {
            ...
            painter.rect_filled(
                bar_rect,
                constants::DATA_BAR_CORNER_RADIUS,
                constants::data_bar_gradient(ui, percentage),
            );
        }
    ...
    response.on_hover_text(format!("{} ({:.1}%)", text, percentage * 100.0));
}
    ...
    fn show_phase_table(
        ...
        let dur_range = |get: fn(&BenchmarkStats) -> Duration| -> (Duration, Duration) {
            entries
                .iter()
                .map(|(_, s)| get(s))
                .fold((Duration::MAX, Duration::ZERO), |(lo, hi), d| {
                    (lo.min(d), hi.max(d))
                })
        };
        ...
        let entries_for_table: Vec<_> = entries
            .iter()
        ...
        ResultsTable::new(&format!("{}_phase_table", section_id), columns).show(
            ui,
            entries_for_table.len(),
            |row_idx, row| {
                let (params, stat) = &entries_for_table[row_idx];

                row.col(|ui| {
                    ui.label(format!("{}", params.implementation));
                });
                row.col(|ui| {
                ...
                row.col(|ui| {
                    draw_duration_with_bar(ui, stat.avg, avg_min, avg_max);
                });
                row.col(|ui| {
                    draw_duration_with_bar(ui, stat.min, min_min, min_max);
                });
                row.col(|ui| {
                    draw_duration_with_bar(ui, stat.max, max_min, max_max);
                });
                row.col(|ui| {
                    draw_duration_with_bar(ui, stat.median, median_min, median_max);
                });
                row.col(|ui| {
                    draw_duration_with_bar(ui, stat.std_dev, std_min, std_max);
                });
            },
        );
    }
